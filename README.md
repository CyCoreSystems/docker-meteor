 ...
 * Bundle-based execution
   ...
   * compressed directory referenced as `COMPRESSED_APP_DIR` (meteor build);
...
define one of `APP_DIR` or `BUNDLE_DIR`:
  ...
  - `COMPRESSED_APP_DIR`
      If you put your compressed (*.tar.gz) application in the directory pointed to by `COMPRESSED_APP_DIR`, this container will attempt to find a compressed Meteor bundle
      ...
      You may attach that as a volume, for greatest simplicity.  Something like: `-e COMPRESSED_APP_DIR='home/meteor/build.tar.gz -v /srv/build.tar.gz:/home/meteor/build.tar.gz`.
  - `BUNDLE_URL`
...
docker run --rm \
  -e ROOT_URL=http://testsite.com \
  -e COMPRESSED_APP_DIR=/home/meteor/build.tar.gz \
